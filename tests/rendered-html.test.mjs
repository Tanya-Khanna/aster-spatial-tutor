...
test("server-renders Aster star's complete product story", async () => {
  ...
  assert.match(html, /Aster-macOS\.zip\?v=0\.3\.2/);
  assert.equal((html.match(/href="\/Aster-macOS\.zip\?v=0\.3\.2"/g) ?? []).length, 3);
...
test("ships a key-required app archive, social card, and removes the starter preview", async () => {
  ...
  assert.match(page, /const downloadHref = "\/Aster-macOS\.zip\?v=0\.3\.2"/);
